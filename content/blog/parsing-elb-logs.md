...
Because of the time delay, monitoring EB logs for real-time alerting is not something I've experimented with processing logs to figure out what went one is something I've spent a lot of time on recently.  It's also a great excuse to glue some command line tools together.

## Getting hold of the logs

First step, pulling down the logs.  These things are often huge and if you've got multiple ELBs it's a pain.  Luckily my colleague John wrote the following neat little one liner to pull the logs for the current hour.
...
CloudWatch will give you a tonne of information about your ELBs and it's not worth you pulling logs, parsing them and doing it yourself.  Check out [the available metrics](https://docs.aws.amazon.com/ElasticLoadBalancing/latest/DeveloperGuide/elb-cloudwatch-metrics.html) before you start.
...
CloudWatch will give you counts of 2xx, 3xx, 4xx and 5xx status codes but sometimes it's important to get the finer detail can be important because 401 and 403 are very different.
...
The index function index($3,":") says "in the text of the 3rd column find the location of the colon".  We then knock off 1 from this value thus turning 217.13.151.25:14990 into 217.13.151.25
...
Another likely candidate for a pre-build analytics tool except where your logs could do with some pre-parsing to get rid of GUIDs and such which make finding groups of URLS a pain.  Here I'm removing add GUIDs of the form 8-4-4-4-12, even when there are multiple in the URL (id and signing for example)
...
There might be, but I've not found something that covers all of these scenarios.  It is very empowering an powerful to be able to quickly glue some simple commands together and get out information for a very specific question such as "which accounts were affected by the gitch in Redis" or "Is something going on that could be affecting latency"
